...
- [ ] Owner visual review of `feat/design-overhaul` (rebased onto merged dev), then merge.
- [ ] Container smoke test, deploy via `scripts/deploy-chanflix-prod.sh`, then post-deploy: re-enable Radarr `tagRequests`, configure the servarr webhook in Radarr/Sonarr Connect (steps in `CHANFLIX_DEV_SERVER.md`), verify Now panel/Operations/Library against live Tautulli/Radarr, watch logs.
