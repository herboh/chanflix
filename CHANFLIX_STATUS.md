...
- [x] `feat/polish` merged to dev (`cedf667`): (1) new "prompt + play" logo (square gruvbox terminal mark, green play-triangle prompt + amber cursor) replacing the purple orb across `logo_full/stacked/os_icon.svg` + regenerated favicons/PWA icons; (2) simple skeleton placeholder card (brand mark, no grey void); (3) download rows now resolve clean titles + posters from the Radarr/Sonarr id (`server/lib/downloadEnrichment.ts`, reused across `/stats/now`, `/downloads`, `/stats/recent`), collapse multi-episode TV into one "Season N · X eps" row, and show exact second-accurate timestamps on hover; (4) Operations Recent Activity de-duplicated (watches/requests only, no longer echoes Recent Completions). Gate: 29/29 unit tests, typecheck clean, production build clean. NOT yet deployed (live still on pre-polish build).
